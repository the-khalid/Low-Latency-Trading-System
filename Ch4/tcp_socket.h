...
struct TCPSocket {
	...
	auto defaultRecvCallback(TCPSocket *socket, Nanos rx_time) noexcept {
		logger_.log("%:% %() % TCPSocket::defaultRecvCallback() socket:% len:% rx:%\n",
				...
				socket->fd_, socket->next_rcv_index_, rx_time);
	}
	...
	TCPSoclet &operator=(const TCPSocket &&) = delete;
	...
	auto sendAndRecv() noexcept -> bool {
		...
		if(n_rcv > 0) {
			...
			if(cmsg->cmsg_level == SOL_SOCKET and cmsg->cmsg_type == SCM_TIMESTAMP and
					cmsg->cmsg_len == CMSG_LEN(sizeof(time_kerenel))) {
				memcpy(&time_kernel, CMSG_DATA(cmsg), sizeof(time_kernel));
				kernel_time = time_kerenel.tv_sec * NANOS_TO_SECS + time_kernel.tv_usec * NANOS_TO_MICROS;
			}

			const auto user_time = getCurrentNanos();

			logger_log("%:% %() % read socket: % len:% utime:% ktime:% diff:%\n",
					__FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_), fd_, next_rcv_valid_index_,
		...
		while(n_send > 0) {
			...
			n_send -= n;
			ASSERT(n == n_send_this_msg,
					"Don't support partial send lengths yet.");
		}
