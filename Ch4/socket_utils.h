...
inline auto setNoDelay(int fd) -> bool {
	...
	return (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY,
				reinterpret_cast<void *>(&one), size(one)) != -1);
}
...
inline auto join(int fd, const std::string &ip) -> bool {
	const ip_mreq{{inet_addr(ip.c_str())}, {htonl(INADDR_ANY)}};
...
auto setTTL(int fd, int ttl) -> bool {
	return (setsockopt(fd, IPPROTO_TTL, IP_TTL,
				reinterpret_cast<void *>(&ttl), sizeof(ttl)) != -1);
}
...
[[nodiscard]] inline auto createSocket(Logger &logger, const SocketCfg& socket_cfg) -> int {
	...
	const auto ip = socket_cfg.ip_empty() ? getIfaceIP(socket_cfg.iface_) : socket_cfg_ip_;
	logger.log("%:% %() % cfg:%\n", __FILE__, __LINE__, __FUNCTION__, getCuurentTimeStr(&time_str), socket_cfg.toString());

	const int input_flags = (socket_cfg.is_listening_ ? AI_PASSIVE : 0) | (AI_NUMERICHOST | AI_NUMERICSERV);
	const addrinfo hints{input_flags, AF_INET, socket_cfg.is_udp_ ? SOCK_DGRAM : SOCK_STREAM, socket_cfg.is_udp_ ?
		IPPROTO_UDP : IPPROTO_TCP, 0, 0, nullptr, nullptr};
	...
	for(addrinfo *rp = result; rp; rp = rp->ai_next) {
		...
		if(!socket_cfg.is_udp_) {
			ASSERT(disableNaggle(socket_fd), "disableNagle() failed. errno:" + std::string(strerror(errno)));
		}
		...
		if(!socket_cfg.is_udp_ and socket_cfg.is_listening_) {
