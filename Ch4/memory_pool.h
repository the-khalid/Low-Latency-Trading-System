	...
	public:
		explicit MemPool(size_t num_elems) : store_(num_elems, {T(), true}) {
			 ASSERT(reinterpret_cast<const ObjectBlock *>(&(store_[0].object_)) == &(store_[0]),
					 "T object should be the first member of ObjectBlock Struct");
		} // this assertion is to handle deallocations
		...
		auto deallocate(const T *elem) noexcept {
			...
			// this casting is OK as we asserted that T is first member in ObjectBlock
			ASSERT(elem_index >= 0 && static_cast<size_t>(elem_index) < store_.size(),
					"Element doesn't belong to this Memory pool");
